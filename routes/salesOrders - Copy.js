...
// ===== START: ROUTES FOR DELIVERY ORDERS (MOVED UP) =====
// GET /for-delivery - ดึงข้อมูล SO ที่พร้อมสำหรับสร้างใบส่งของ (ต้องอยู่ก่อน /:id)
...
// GET /:id/delivery-items - ดึงรายการสินค้าที่เหลือสำหรับจัดส่ง (ต้องอยู่ก่อน /:id)
router.get('/:id/delivery-items', (req, res) => {
    const { id } = req.params;
    const soItemsSql = `
        SELECT 
        ...
        WHERE soi.so_id = ?
    `;
    db.all(soItemsSql, [id], (err, soItems) => {
        ...
        `;
        db.all(shippedItemsSql, [id], (err, shippedItems) => {
            if (err) return res.status(500).json({ success: false, error: err.message });
...
// ===== END: ROUTES FOR DELIVERY ORDERS =====
