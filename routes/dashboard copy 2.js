...
router.get('/main-data', async (req, res) => {
    try {
        ...
        const responseData = {
            ...
            sales: {
                total_sales: kpi_sales[0]?.total || 0,
                new_quotes: 0,
                new_orders: sales_kpi_orders[0]?.count || 0,
                quote_table: []
            },
            inventory: {
...
router.get('/machine-view/:machineCode', async (req, res) => {
    ...
    try {
        ...
        const inProgressJob = await all(`
            ...
            (SELECT SUM(good_quantity) FROM production_records WHERE mo_id = mo.id) as total_good,
            (SELECT SUM(total_scrap_quantity) FROM production_records WHERE mo_id = mo.id) as total_scrap
        ...
        const pendingJobs = await all(`
            SELECT mo_number, item_code, quantity_to_produce FROM manufacturing_orders
            WHERE machine_id = ? AND status = 'pending' ORDER BY due_date ASC LIMIT 10;`, [machineId]);
        const defectCodes = await all("SELECT id, code, description FROM defect_codes WHERE is_active = 1 ORDER BY code");
        res.json({ success: true, data: { machine: machineInfo[0], inProgressJob: inProgressJob[0] || null, pendingJobs, defectCodes }});
    } catch (error) { res.status(500).json({ success: false, error: "API Error in /machine-view" }); }
});
...
router.post('/start-specific-job', async (req, res) => {
    ...
    try {
        ...
        if (runningJob.length > 0) {
            return res.status(409).json({ success: false, error: `เครื่องจักรกำลังทำงาน ${runningJob[0].mo_number} อยู่` });
        }
...
router.post('/record-production', async (req, res) => {
    const { mo_number, good_quantity, scraps, operator_name = "Operator" } = req.body;
    if (!mo_number || good_quantity === undefined || !scraps) {
...
router.get('/mo-details/:moNumber', async (req, res) => {
    ...
    try {
        ...
        const production_records = await all(`
            ...
            ORDER BY record_date DESC`, [moNumber]);
        res.json({
            success: true,
...
// +++ START: API ใหม่สำหรับตรวจสอบตารางล่าสุดของเครื่องจักร +++
router.get('/machines/:id/latest-schedule', async (req, res) => {
    ...
    try {
        ...
        res.json({
            success: true,
            data: {
                latest_end_time: result[0]?.latest_end_time || null
            }
        });
    } catch (error) {
...
// +++ END: API ใหม่ +++
// +++ START: แก้ไข API สำหรับ Machine Dashboard Overview +++
router.get('/machines-overview', async (req, res) => {
    try {
        const query = `
            ...
            ORDER BY m.machine_code ASC; -- จัดเรียงตามรหัสเครื่องจักร
        `;
